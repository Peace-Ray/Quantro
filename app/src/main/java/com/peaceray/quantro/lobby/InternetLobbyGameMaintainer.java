 ...
 *
 */
public class InternetLobbyGameMaintainer {
	...
	private static final String TAG = "InternetLobbyGameMaintainer" ;
	
	InternetLobbyGameMaintainerThread mThread ;
	InternetLobbyGame mGame ;
	WeakReference<Delegate> mwrDelegate ;
	boolean mRefreshOnly ;
	...
	public static InternetLobbyGameMaintainer newRefresher( InternetLobbyGame ilg, Delegate d ) {
		return new InternetLobbyGameMaintainer( ilg, d, true ) ;
	}
	
	public static InternetLobbyGameMaintainer newMaintainer( InternetLobbyGame ilg, Delegate d ) {
		return new InternetLobbyGameMaintainer( ilg, d, false ) ;
	}
	
	
	private InternetLobbyGameMaintainer( InternetLobbyGame ilg, Delegate d, boolean refreshOnly ) {
		if ( ilg == null )
			throw new NullPointerException("Null game given") ;
	...
	synchronized public InternetLobbyGameMaintainer hostEvery( long millis ) {
		if ( mRefreshOnly )
			throw new IllegalStateException("This InternetLobbyGameMaintainer created as a Refresher, not a Maintainer") ;
		mHostEvery = millis ;
	...
	synchronized public InternetLobbyGameMaintainer maintainEvery( long millis ) {
		if ( mRefreshOnly )
			throw new IllegalStateException("This InternetLobbyGameMaintainer created as a Refresher, not a Maintainer") ;
	...
	synchronized public InternetLobbyGameMaintainer refreshEvery( long millis ) {
	...
	synchronized public InternetLobbyGameMaintainer host() {
		if ( mRefreshOnly )
			throw new IllegalStateException("This InternetLobbyGameMaintainer created as a Refresher, not a Maintainer") ;
	...
	synchronized public InternetLobbyGameMaintainer maintain() {
		if ( mRefreshOnly )
			throw new IllegalStateException("This InternetLobbyGameMaintainer created as a Refresher, not a Maintainer") ;
		if ( mThread == null || mThread.mHandler == null )
	...
	synchronized public InternetLobbyGameMaintainer refresh() {
		if ( mThread == null || mThread.mHandler == null )
	...
	synchronized public void start() {
		...
		mThread = new InternetLobbyGameMaintainerThread() ;
		mThread.start() ;
	...
	private class InternetLobbyGameMaintainerThread extends Thread {
		...
		private void noteSuccess( int action ) {
			//Log.d(TAG, "InternetLobbyGameMaintainer: note success " + action) ;
			Delegate d = mwrDelegate.get() ;
			if ( d != null )
				d.ilgmd_communicationSuccess(
						InternetLobbyGameMaintainer.this,
						action,
		...
		private long noteFailureAndGetDelay( int action, CommunicationErrorException cee ) {
			//Log.d(TAG, "InternetLobbyGameMaintainer: note failure " + action) ;
			//cee.printStackTrace() ;
			long delay = 0 ;
			Delegate d = mwrDelegate.get() ;
			if ( d != null )
				delay = d.ilgmd_retryDelayAfterCommunicationFailure(
						InternetLobbyGameMaintainer.this,
						action,
						cee.getError(),
						cee.getErrorReason(),
		...
		public void run() {
			...
			//Log.d(TAG, "InternetLobbyGameMaintainer: run()") ;
			...
			mHandler = new Handler() {
				...
				public void handleMessage (Message msg) {
					...
					//Log.d(TAG, "InternetLobbyGameMaintainer: handleMessage") ;
					
					long delay = 0 ;
					
					switch( msg.what ) {
					case MESSAGE_TYPE_STOP:
						//Log.d(TAG, "InternetLobbyGameMaintainer: STOP") ;
						getLooper().quit() ;
						break ;
						
					case MESSAGE_TYPE_HOST:
						//Log.d(TAG, "InternetLobbyGameMaintainer: HOST") ;
						if ( mRefreshOnly )
							break ;
						removeMessages(MESSAGE_TYPE_HOST) ;
						try {
							// host, then schedule the next hosting message.
							if ( mGame.getStatus() == WebConsts.STATUS_EMPTY )
								mGame.refresh() ;
							else if ( mGame.getStatus() == WebConsts.STATUS_OPEN )
								mGame.host() ;
					...
					case MESSAGE_TYPE_MAINTAIN:
						//Log.d(TAG, "InternetLobbyGameMaintainer: MAINTAIN") ;
						if ( mRefreshOnly )
							break ;
						...
						if ( mGame.getStatus() == WebConsts.STATUS_EMPTY ) {
							...
							try {
								if ( mMaintenanceWorkOrder == null ) {
									mMaintenanceWorkOrder = mGame.maintainRequest() ;
									...
									// report.
									mGame.maintainConfirm(mMaintenanceWorkOrder) ;
									noteSuccess( Delegate.ACTION_MAINTAIN ) ;
					...
					case MESSAGE_TYPE_REFRESH:
						//Log.d(TAG, "InternetLobbyGameMaintainer: REFRESH") ;
